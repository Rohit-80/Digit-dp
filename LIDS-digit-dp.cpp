...
string l,r;
ll lenx;
ll dpl[11][11][11][2][2][2];
ll dplen(ll pos,bool t1,bool t2, ll len, ll c,ll str){
    
    if(pos == r.size()) return len ;


      if(dpl[pos][c+1][len][t1][t2][str] != -1) return dpl[pos][c+1][len][t1][t2][str];
      ll st = t1 ? l[pos] - '0' : 0;
      ll en = t2 ? r[pos] - '0' : 9;
     ...
     ll res = 0;
      ...
      for(ll i = st; i <= en; i++){
            ...
            res = max(res, dplen(pos + 1 , t1&(i==st) , t2&(i==en),len,c,str|(i > 0)));
            if(c < i && (str | (i>0))){
             
            res = max(res, dplen(pos + 1 , t1&(i==st) , t2&(i==en), len+1,i,str|(i > 0)));}
      } 

      return dpl[pos][c+1][len][t1][t2][str] = max(dpl[pos][c+1][len][t1][t2][str],res);

}

ll dp[11][11][11][2][2][2];
ll rec(ll pos,bool t1,bool t2, ll len, ll c,ll str,ll lastdigit){
    // trace(pos,str,s,len);
      if(pos == r.size()){
      ...
      if(dp[pos][lastdigit+1][len][t1][t2][str]!= -1) return dp[pos][lastdigit+1][len][t1][t2][str];
      ...
      for(ll i = st; i <= en; i++){
           ll val = i;
           if(str == 0 and i == 0) val = 1;

           res += rec(pos + 1 , t1&(i==st) , t2&(i==en), len,val,str|(i>0),lastdigit);
            if(lastdigit < i&& len <= lenx && (str | (i>0))){
                ll inf = len + 1;
                // if(str) inf += 1;
              res += rec(pos + 1 , t1&(i==st) , t2&(i==en), inf,val,str|(i>0),i);
            }
      }
      return dp[pos][lastdigit+1][len][t1][t2][str] = res;
}
...
void test(){
     ...
     all(l);
      ...
      lenx = dplen(0,1,1,0,-1,0);
      
       memset(dp,-1,sizeof(dp));
      ll ans = rec(0,1,1,0,-1,0,-1);
