...
ll dp[2010][2010][2][2][2];

ll rec(ll pos,ll tight,ll taken,ll st,ll even){
    // trace(pos,taken);
    if(pos == s.size()) {  return (taken == 0); }

     if(dp[pos][taken][tight][st][even] != -1) return dp[pos][taken][tight][st][even];
    ll res = 0;
    ...
    for(ll i = 0 ; i <= en; i++){
         ...
         if(st == 0 && i == 0){
            res = (res%mod + rec(pos+1,tight&(i==en),(taken*10 + i)%m,st,even)%mod)%mod;   
         }else{
             if(even && i == d)
             res =  (res%mod + rec(pos+1,tight&(i==en),(taken*10 + i)%m,st|(i>0),!even)%mod)%mod;   
             else if(!even && i != d)
             res =  (res%mod + rec(pos+1,tight&(i==en),(taken*10 + i)%m,st|(i>0),!even)%mod)%mod;   
         }
         
    }
    return dp[pos][taken][tight][st][even] = res%mod;
}
...
void test(){
     ...
     cin>>l>>r;
       ...
       ll ans = rec(0,1,0,0,0);
       // trace(ans);
         s = l;
       memset(dp,-1,sizeof(dp)); 
        ans = (ans%mod - rec(0,1,0,0,0)%mod +mod)%mod;
      
        bool check = true;
        ll sum = 0;
       for(int i = 0; i < l.length(); i++){
           sum = ((sum*10)%m + (l[i]-'0')%m)%m;
           if(i%2 == 0 && (l[i]-'0') == d){
                check = false;
           }else if(i%2 == 1 && (l[i]-'0') != d){
               check = false;
           }
       }
